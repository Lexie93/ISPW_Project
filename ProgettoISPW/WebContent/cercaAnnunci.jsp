...
<form action="cercaAnnunci.jsp" name="myform" method="POST" >
		...
		<% if (request.getParameter("Vai")!=null && request.getParameter("ricerca").trim()!="")  {
			BeanRicerca.setRicerca(request.getParameter("ricerca"));
			BeanRicerca.cerca();
		...
		<table style="background-color:PowderBlue">
			<%for (int i=0;i<BeanRicerca.getNum_articoli();i++){%>
				...
				<td><a  href='http://localhost:8080/ProgettoISPW/aggiungiArticoloPreferito.jsp?id=<%=BeanRicerca.getArticolo(i).getID()%>'>Aggiungi alla lista desideri</a></td>
				</tr>
			<%}%>
		</table>
		<%} else { %>
